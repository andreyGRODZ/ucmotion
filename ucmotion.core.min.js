...
(function (w,d){'use strict';
  ...
  const DEFAULTS = {
    ...
    // ↓ безопасные маски/курсив (теперь дефолт = 0, bleed вручную)
  ...
  const UCM={
    ...
    init(selector){
      ...
      entries.forEach(({sel,el})=>{
        ...
        const cfg=this.util.readTogglesFrom(el, CONFIG);
        try { REGISTRY.get(sel).prepare(el,cfg,this.util); } catch(e){ console.warn('[UCM] prepare', e); }
        const start=()=>{ if(el.__ucRan) return; if(!atom.__ucLinesBuilt){ requestAnimationFrame(start); return; } el.__ucRan=true; neutralizeBasic(el); try { REGISTRY.get(sel).run(el,cfg,this.util); } catch(e){ console.warn('[UCM] run', e); } };
        const delay = cfg.preferBasicDelay ? this.util.readBasicDelay(el, cfg.baseDelay) : cfg.baseDelay;
    ...
    util:{ready,getAtom:getAtomSafe,toMs,hasBasic,readBasicDelay,neutralizeBasic,injectBasicNeutralizerFor,/* … остальное из v1.3.1 */},
  ...
  (function banner(){ if(w.__UCM_BANNER__)return; w.__UCM_BANNER__=true; try{console.log('%cUC Motion v'+UCM.version+' — by '+UCM.author+'  |  '+UCM.repository,'color:#25EDFF;font-weight:600;');}catch(_){} })();
