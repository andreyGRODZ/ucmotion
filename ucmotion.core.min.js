...
<!-- UC Motion Addon: FontWarmup + SmartFontWait -->
<script>
(function(w,d){
  'use strict';
  if(!w.UCM) return;
  ...
  function waitForFonts(timeout){
    ...
    const fastReady=()=>{
      ...
      try{if(fam&&document.fonts.check(`24px ${fam}`)){sessionStorage.setItem('ucFontsReady','1');return true;}}catch(_){}
      return false;
    };
    ...
    const readyP=document.fonts.ready.then(()=>{sessionStorage.setItem('ucFontsReady','1');}).catch(()=>{});
    const event=new Promise(res=>{
      try{document.fonts.addEventListener('loadingdone',()=>{sessionStorage.setItem('ucFontsReady','1');res();},{once:true});}catch(_){res();}
    });
  ...
  try{console.log('%cUCM Addon v1.2.0: FontWarmup + SmartFontWait enabled','color:#25EDFF');}catch(_){}
})(window,document);
